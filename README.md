...
The implemented product and visual contract is documented in [`design.md`](design.md). A portable, model-facing version of the visual system is available in [`theme.md`](theme.md). The current reference comparison, browser evidence, resolved findings, and testing limits are recorded in [`docs/qa/AUDIT.md`](docs/qa/AUDIT.md).
