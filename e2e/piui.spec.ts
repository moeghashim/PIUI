...
test("runs the complete PIUI workflow and survives reload", async ({ page }, testInfo) => {
  ...
  await expect(page.getByText("3 matching extensions")).toBeVisible();
  await expect(page.getByRole("link", { name: "pi-marketplace-tools", exact: true })).toBeVisible();
  await expect(page.getByText("installed", { exact: true })).toBeVisible();
  await expectComfortableSearch(page.getByLabel("Search extensions"));
  await page.screenshot({ path: `docs/qa/piui/marketplace-${testInfo.project.name}.png`, fullPage: false });
  await page.getByLabel("Search extensions").fill("tools");
  await expect(page.getByText("1 matching extension")).toBeVisible();
  await expect(page.getByRole("link", { name: "pi-marketplace-tools", exact: true })).toBeVisible();
  await page.getByLabel("Search extensions").fill("");
  await expect(page.getByText("3 matching extensions")).toBeVisible();
  await page.getByRole("button", { name: "Next" }).click();
  await expect(page.getByRole("link", { name: "@fixture/pi-companion", exact: true })).toBeVisible();
  await page.getByRole("tab", { name: /Installed/ }).click();
  await expect(page.locator(".inventory-summary").getByText("configured sources")).toBeVisible();
